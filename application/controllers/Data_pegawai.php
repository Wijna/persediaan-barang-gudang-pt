...
class Data_pegawai extends CI_Controller
{
    ...
    public function tambah_pegawai()
    {
        ...
        if ($this->input->post('submit', TRUE) == 'submit') {
            ...
            $this->form_validation->set_rules(
                'username',
                'Username',
                "required|min_length[3]|max_length[20]|is_unique[tbl_user.username]|regex_match[/^[A-Za-z0-9]+$/]",
                array(
    ...
    public function hapus_data()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                //tangkap rowid
                $id = $this->security->xss_clean($this->input->post('id_user', TRUE));

                $hapus = $this->m_pegawai->delete('tbl_user', ['id_user' => $id]);
    ...
    public function ajax_pegawai()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($list as $i) {
                ...
                <button type="button" class="btn btn-danger btn-sm"onclick="hapus_pegawai(\'' . $i->id_user . '\')">Hapus</button>';
