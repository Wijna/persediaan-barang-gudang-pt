...
<div class="row">
    <div class="col-sm-12 col-md-10">
        <h4 class="mb-0"><i class="fa fa-file-text"></i> Laporan Bulanan Barang Masuk</h4>
    </div>
</div>
<hr class="mt-0" />
...
<div class="row">
    ...
    <div class="col-md-2 col-sm-12">
        <a href="<?= site_url('barang_masuk_bulanan/' . $bulan . '-' . $tahun); ?>" class="btn btn-success btn-block btn-sm" target="_blank">
            <i class="fa fa-print"></i> Cetak Laporan
        </a>
        <a href="<?= site_url('barang_masuk_bulanan/export_excel/' . $bulan . '-' . $tahun); ?>" class="btn btn-info btn-block btn-sm" target="_blank">
        ...
        if ($data->num_rows() > 0) {
            ...
            foreach ($data->result() as $dt) {
                ...
                if ($row_tanggal == 1) :
                    ...
                    echo '<td rowspan="' . $dt->row_tanggal . '">' . tanggal_indo($dt->tgl_barang_masuk) . '</td>';
                endif;
                if ($row_barang_masuk == 1) :

                    echo '<td rowspan="' . $dt->row_barang_masuk . '">' . $dt->id_barang_masuk . '</td>';
                    echo '<td rowspan="' . $dt->row_barang_masuk . '">' . $dt->nama_supplier . '</td>';
                endif;
                ...
                echo '</tr>';
                if ($row_barang_masuk != $dt->row_barang_masuk) {
                    $row_barang_masuk++;
                } else {
                    $row_barang_masuk = 1;
                }
