...
<head>
    ...
    <title><?= $title; ?></title>
    <link rel="shortcut icon" href="<?= base_url('assets/img/jms4.ico'); ?>">
    <!-- Bootstrap CSS -->
...
<body>
 ...
 title: 'Apakah anda yakin akan keluar ?', type: 'warning' , showCancelButton: true, confirmButtonColor: '#DD6B55' , confirmButtonText: 'Ya, Logout!', closeOnConfirm: false }, function(){ window.location.href = '<?= site_url('sign_out'); ?>'; });">Logout</a>
                                </div>
                            </li>
                        </ul>
                    </div>
    ...
    <!-- Custom Javascript -->
    <?php
    $arr_uri = array('stok_barang', 'barang', 'jenis_barang', 'satuan_barang', 'lokasi_barang', 'pegawai', 'supplier', 'data_barang_masuk', 'data_barang_keluar');
    ...
    if (in_array(strtolower($this->uri->segment(1)), $arr_uri) && !$this->uri->segment(2)) :
        switch ($this->uri->segment(1)) {
            ...
            case 'data_barang_masuk':
                $file = 'ajax_barang_masuk';
                break;
            case 'data_barang_keluar':
                $file = 'ajax_barang_keluar';
                break;
        }
    ?>
        <script>
    ...
    <script>
        ...
        function hapus_barang_masuk(id) {
            swal({
                ...
                $.ajax({
                    url: "<?= site_url('hapus_barang_masuk'); ?>",
                    method: "POST",
                    data: {
                        id: id,
                        csrf_token: csrf_token
        ...
        $('.pilih-barang').change(function() {
            ...
            $.ajax({
                url: "<?= site_url('cari_barang_barang_keluar'); ?>",
                method: "POST",
                data: {
                    id: id,
                    csrf_token: csrf_token
        ...
        function tambah_barang_keluar() {
            var id = $('#barang-barang_keluar').val();
            var sisa = $('#sisa').val();
            var qty = $('#jumlahx').val();
            var csrf_token = Cookies.get('csrf_cookie');
            $('#barang-barang_keluar').addClass('pilih-barang');
            ...
            $.ajax({
                url: "<?= site_url('tambah_cart_barang_keluar'); ?>",
                method: "POST",
                data: {
                    id: id,
                    qty: qty,
        ...
        function hapus_item_barang_keluar(rowid) {
            $('#barang-barang_keluar').addClass('pilih-barang')
                .removeAttr('disabled');
            ...
            swal({
                ...
                $.ajax({
                    url: "<?= site_url('hapus_item_barang_keluar'); ?>",
                    method: "POST",
                    data: {
                        rowid: rowid,
                        csrf_token: csrf_token
                    ...
                    success: function(obj) {
                        ...
                        if (x.message == 'success') {
                            swal({
                                ...
                                $('#sisa').val('');
                                $('#jumlahx').val('');
                                $('#btn-act').html('<button type="button" class="btn btn-success btn-sm tambah-barang_keluar" onclick="tambah_barang_keluar()">Tambah Barang</button>');
        ...
        function get_item_barang_keluar(rowid) {
            $('#barang-barang_keluar').removeClass('pilih-barang');
            var csrf_token = Cookies.get('csrf_cookie');

            $.ajax({
                url: "<?= site_url('get_item_barang_keluar/' . $this->uri->segment(1)); ?>",
                method: 'POST',
                data: {
                    rowid: rowid,
                    csrf_token: csrf_token
                ...
                success: function(data) {
                    ...
                    if (x.status == 'true') {
                        ...
                        $('#jumlahx').focus();
                        $('#btn-act').html('<button type="button" class="btn btn-success btn-sm" onclick="update_cart_barang_keluar()">Update Barang</button>');
                    }
                },
        ...
        function update_cart_barang_keluar() {
            var qty = $('#jumlahx').val();
            ...
            $.ajax({
                url: "<?= site_url('update_cart_barang_keluar'); ?>",
                method: "POST",
                data: {
                    id: id,
                    jumlah: qty,
        ...
        function hapus_barang_keluar(id) {
            swal({
                ...
                $.ajax({
                    url: "<?= site_url('hapus_barang_keluar'); ?>",
                    method: "POST",
                    data: {
                        id: id,
                        csrf_token: csrf_token
                ...
                $.ajax({
                    ...
                    success: function(obj) {
                        ...
                        if (a.message == 'success') {
                            swal({
                                title: "Success!",
                                text: "Data jenis berhasil dihapus",
                                type: "success",
                                showCancelButton: false,
                                showConfirmButton: false,
                                timer: 2000
